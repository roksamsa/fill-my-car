...
import { AllTripsListWithFilterService } from './all-trips-list-with-filter.service';

@Component({
  selector: 'app-all-trips-list-with-filter',
  templateUrl: './all-trips-list-with-filter.component.html',
  styleUrls: ['./all-trips-list-with-filter.component.scss']
})
export class AllTripsListWithFilterComponent implements OnInit, OnDestroy {

  fetchTripsSubscription: Subscription;
  fetchSearchTripsSubscription: Subscription;
  fetchVehiclesSubscription: Subscription;
  deleteTripSubscription: Subscription;

  public areThereAnyTrips = false;
  public trips: Trip[] = [];
  public tripsSearchArray: Trip[] = [];
  ...
  constructor(
    ...
    private tripSearchValue: AllTripsListWithFilterService,
  ...
  ngOnInit() {
    ...
    this.tripSearchValue.currentSearchValue.subscribe(value => {
      this.tripSearchInputValue = value;
      this.fetchSearchTrips(value);
    });
  }
  ...
  // Fetch all trips with search
  fetchSearchTrips(searchValue) {
    this.fetchSearchTripsSubscription = this.tripService.getTripByTripName(searchValue).subscribe((data: any) => {
      console.log(data);
      if (data !== null) {
        this.tripsSearchArray = data;
        this.areThereAnyTrips = true;
        this.preloadingSpinnerShow();
        this.selectedVehicleId = data[0].selectedVehicle;
        this.fetchVehicle(this.selectedVehicleId);
        ...
        this.fetchTrips();
        this.tripsSearchArray = this.trips;
        this.areThereAnyTrips = true;
        this.preloadingSpinnerShow();
      }
    });
  ...
  ngOnDestroy() {
    ...
    if (this.fetchSearchTripsSubscription) {
      this.fetchSearchTripsSubscription.unsubscribe();
    }
  }
}
